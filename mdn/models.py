...
class MixtureDensity(nn.Module):
    ...
    def loss(self, x, y):
        pi, normal = self.forward(x)  # Nx*xC, Nx*xCxO

        loglik = normal.log_prob(y.unsqueeze(-2).expand_as(normal.loc))  # Nx*xCxO
        loglik = torch.sum(loglik, dim=-1)  # Nx*xC
        loss = -torch.logsumexp(torch.log(pi.probs) + loglik, dim=-1)  # Nx*
        return loss.mean()
